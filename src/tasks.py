...
def plan_automation_sequence_task(agent, artifacts_dir: str) -> Task:
    description = f"""
    Build a global execution plan before YAML implementation starts.
    Read pending scenarios via qase_parser and summarize current blockers or flaky hotspots
    using state_tracker + app_flow_memory memory. Prioritize which scenario should be automated
    now, and explain why this order reduces risk and rework.
    ...
    Keep output compact and actionable so downstream agents can execute immediately.
    """
...
def automate_tests_task(agent, app_path: str, artifacts_dir: str, max_attempts: int) -> Task:
    ...
    Before writing flow for each case, consult manager priorities from
    `{artifacts_dir}/manager_plan.json`, then consult the latest AppFlow plan file inside
